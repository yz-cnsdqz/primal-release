...
###############################################################################
# ARDiffusion Action Generation Pipeline
#
# This script provides a complete pipeline for:
# 1. Generating action-based motion sequences from trained models
# 2. Rendering generated motions as videos
# 3. Evaluating motion quality metrics
#
# Usage:
#   bash scripts/run_ARDiffusionAction.sh <mode> [checkpoint_dir]
#
# Modes:
#   gen    - Generate motion sequences for all actions
#   render - Render videos from generated sequences
#   eval   - Evaluate motion quality metrics
#
# Example:
#   bash scripts/run_ARDiffusionAction.sh gen logs/primal/motion_diffuser_ar_action/runs/2025-02-22_21-24-07_ours
###############################################################################
...
readonly GENERATION_PARAMS="--ckptidx 999 --use_ema --dataset SFU --use_inertialization --use_reproj_kpts --frame_skip 400"
readonly OUTPUT_DIR="outputs/ARDiffusionAction"
...
if [ $# -ne 2 ]; then
    ...
    echo "  gen    - Generate motion sequences for all actions"
    echo "  render - Render videos from generated sequences"
    echo "  eval   - Evaluate motion quality metrics"
    echo ""
    echo "Example:"
    echo "  $0 gen logs/primal/motion_diffuser_ar_action/runs/2025-02-22_21-24-07_ours"
    exit 1
fi
...
echo "========================================="
echo "ARDiffusion Action Generation Pipeline"
echo "Mode: $MODE"
...
###############################################################################
generate_motions() {
    echo "Starting action motion generation..."

    local total_combinations=$((${#EXPERIMENTS[@]} * ${#ACTIONS[@]}))
    local current=0
    ...
    for expdir in "${EXPERIMENTS[@]}"; do
        ...
        for action in "${ACTIONS[@]}"; do
            current=$((current + 1))
            echo "[$current/$total_combinations] Generating action '$action' for $exp_name"

            $PYTHON_CMD experiments/gen_Motion_ARDiffusionAction.py \
                --expdir "$expdir" \
                --action "$action" \
                $GENERATION_PARAMS

            echo "✓ Completed generation for action '$action'"
        done

        echo "✓ Completed all actions for $exp_name"
    done

    echo "All action motion generation completed!"
}
...
render_videos() {
    ...
    for expdir in "${EXPERIMENTS[@]}"; do
        ...
        for action in "${ACTIONS[@]}"; do
            current=$((current + 1))
            echo "[$current/$total_combinations] Rendering action '$action' for $exp_name"

            local pkl_file="$OUTPUT_DIR/$exp_name/SFU_ema-True_reproj-True_inertial-True_action-$action.pkl"

            if [ ! -f "$pkl_file" ]; then
                echo "Warning: PKL file not found: $pkl_file"
                echo "Skipping rendering for action '$action'"
                continue
            fi

            $PYTHON_CMD primal/rendering/render.py "$pkl_file" false
            echo "✓ Completed rendering for action '$action'"
        done
...
evaluate_metrics() {
    ...
    for expdir in "${EXPERIMENTS[@]}"; do
        ...
        for action in "${ACTIONS[@]}"; do
            current=$((current + 1))
            echo "[$current/$total_combinations] Evaluating action '$action' for $exp_name"

            local pkl_file="$OUTPUT_DIR/$exp_name/SFU_ema-True_reproj-True_inertial-True_action-$action.pkl"
            local output_file="$OUTPUT_DIR/$exp_name/SFU_ema-True_reproj-True_inertial-True_action-$action.txt"

            if [ ! -f "$pkl_file" ]; then
                echo "Warning: PKL file not found: $pkl_file"
                echo "Skipping evaluation for action '$action'"
                continue
            fi
            ...
            echo "✓ Completed evaluation for action '$action'"
        done
...
echo "Experiment: $EXPERIMENT_NAME"
echo "Actions processed: ${ACTIONS[*]}"
echo "========================================="
