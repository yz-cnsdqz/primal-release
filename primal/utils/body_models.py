...
class SMPLXROTMAT(nn.Module):
    ...
    def __init__(self,
        ...
        super(SMPLXROTMAT, self).__init__()
        ...
        # root_orient
        # if self.model_type in ['smpl', 'smplh']:
        # init_root_orient_aa = torch.zeros((1,3), dtype=dtype)
        # init_root_orient = batch_rodrigues(
        #                     init_root_orient_aa.view(-1, 3), 
        #                     dtype=dtype).view([1, -1, 3, 3])
        # breakpoint()
        # breakpoint()
        idty = torch.tensor([[[[1., 0., 0.],
                [0., 1., 0.],
                [0., 0., 1.]]]], dtype=dtype)
        self.comp_register('init_root_orient', idty,persistent=persistant_buffer)

        

        # pose_body
        if self.model_type in ['smpl', 'smplh', 'smplx']:
            self.comp_register('init_pose_body',idty.repeat(1,63,1,1).clone(), persistent=persistant_buffer)
        elif self.model_type == 'animal_horse':
            self.comp_register('init_pose_body', torch.zeros((1,105), dtype=dtype), persistent=persistant_buffer)
        elif self.model_type == 'flame':
            self.comp_register('init_pose_body', torch.zeros((1,3), dtype=dtype), persistent=persistant_buffer)
        ...
        if self.model_type in ['smpl']:
            ...
            # init_pose_hand_aa = torch.zeros((1,15*3*2), dtype=dtype)
            # init_pose_hand = batch_rodrigues(init_pose_hand_aa.view(-1, 3), dtype=dtype).view([1, -1, 3, 3])
            self.comp_register('init_pose_hand', idty.repeat(1,30,1,1).clone(), persistent=persistant_buffer)
        elif self.model_type in ['mano']:
            self.comp_register('init_pose_hand', torch.zeros((1,15*3), dtype=dtype), persistent=persistant_buffer)

        # face poses
        if self.model_type in ['smplx','flame']:
            # init_pose_jaw_aa = torch.zeros((1,1*3), dtype=dtype)
            # init_pose_jaw = batch_rodrigues(
            #                     init_pose_jaw_aa.view(-1, 3), 
            #                     dtype=dtype).view([1, -1, 3, 3])
            self.comp_register('init_pose_jaw', idty.clone(), persistent=persistant_buffer)
            # init_pose_eye_aa = torch.zeros((1,2*3), dtype=dtype)
            # init_pose_eye = batch_rodrigues(
            #                     init_pose_eye_aa.view(-1, 3), 
            #                     dtype=dtype).view([1, -1, 3, 3])
            self.comp_register('init_pose_eye',idty.repeat(1,2,1,1).clone(), persistent=persistant_buffer)
    ...
    def forward(self, root_orient=None, pose_body=None, pose_hand=None, pose_jaw=None, pose_eye=None, betas=None,
        ...
        assert self.model_type in ['smpl', 'smplh', 'smplx', 'mano', 'animal_horse', 'animal_dog', 'flame', 'animal_rat'], ValueError(
            'model_type should be in smpl/smplh/smplx/mano')
        if root_orient is None:  root_orient = self.init_root_orient.expand(batch_size, -1,-1,-1)
        if self.model_type in ['smplh', 'smpl']:
            if pose_body is None:  pose_body = self.init_pose_body.expand(batch_size, -1)
            if pose_hand is None:  pose_hand = self.init_pose_hand.expand(batch_size, -1)
        elif self.model_type == 'smplx':
            if pose_body is None:  pose_body = self.init_pose_body.expand(batch_size, -1)
            if pose_hand is None:  pose_hand = self.init_pose_hand.expand(batch_size, -1,-1,-1)
            if pose_jaw is None:  pose_jaw = self.init_pose_jaw.expand(batch_size, -1,-1,-1)
            if pose_eye is None:  pose_eye = self.init_pose_eye.expand(batch_size, -1,-1,-1)
        elif self.model_type == 'flame':
            if pose_body is None:  pose_body = self.init_pose_body.expand(batch_size, -1)
            if pose_jaw is None:  pose_jaw = self.init_pose_jaw.expand(batch_size, -1)
            if pose_eye is None:  pose_eye = self.init_pose_eye.expand(batch_size, -1)
        ...
        if pose_hand is None and self.model_type not in ['animal_horse', 'animal_dog', 'animal_rat','flame']:
            pose_hand = self.init_pose_hand.expand(batch_size, -1,-1,-1)
        ...
        if self.model_type == 'smplx':
            full_pose = torch.cat([root_orient, pose_body, pose_jaw, pose_eye, pose_hand], dim=1)  # orient:3, body:63, jaw:3, eyel:3, eyer:3, handl, handr
            full_pose = full_pose.reshape(batch_size,-1,9)
        else:
