...
def predict_mop(args) -> None:
    ...
    for batch in tqdm(dataloader):
        ...
        result_one = {
            ...
            'action_label': batch["action_label"].detach().cpu().numpy(),
            'xs': xs.detach().cpu().numpy()
        }
        results_all.append(result_one)
    
    outputdir = os.path.join('outputs/ARDiffusionAction', f'{os.path.basename(args.expdir)}')
    os.makedirs(outputdir, exist_ok=True)
    outputfile = os.path.join(
        outputdir,
        f'{args.dataset}_ema-{args.use_ema}_reproj-{args.use_reproj_kpts}_inertial-{args.use_inertialization}_action-{args.action}.pkl')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action", 
                        help="which action to generate? ['jump', 'kick', 'punch', 'run', 'walk']",
                        default=None)
    parser.add_argument("--n_frames", type=int, default=110,)
    ...
    ori = ori.float().to(model.device) * 1.0
