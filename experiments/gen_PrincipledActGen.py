...
def predict_mop(args) -> None:
    ...
    for batch in tqdm(dataloader):
        ...
        betas, xb_gen, _, _ = model.generate_perpetual_navigation(
            ...
            perform_principled_action = args.action,
        )
        xb_gen = xb_gen[0]
    ...
    outputdir = os.path.join('outputs', f'PrincipledActionGen/{os.path.basename(args.expdir)}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action", 
                        help="which action to generate? [left_kick, right_kick,leap_forward, flip_back, roll_forward ]",
                        default=None)
    ...
    assert args.action in [None, 'left_kick', 'right_kick', 'run_forward', 'flip_back', 'roll_forward'], 'Action must be [left_kick, right_kick,run_forward, flip_back, roll_forward]'
