...
def predict_mop(args) -> None:
    ...
    for batch in tqdm(dataloader):
        ...
        result_one = {
            ...
            'tgt_locs': ori.detach().cpu().numpy()
        }
        results_all.append(result_one)
    
    outputdir = os.path.join('outputs/Res_ARDiffusionSpatial', f'{os.path.basename(args.expdir)}')
    os.makedirs(outputdir, exist_ok=True)
    outputfile = os.path.join(
        outputdir,
        f'{args.dataset}_ema-{args.use_ema}_reproj-{args.use_reproj_kpts}_inertial-{args.use_inertialization}_dir-{args.target_dir}_dist-{args.target_dist}.pkl')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_dir", type=float,
                        ...
                        required=True)
    parser.add_argument("--target_dist", type=float,
                        help="the target distance",
                        required=True)
    ...
    ori = ori.float().to(model.device) * args.target_dist
