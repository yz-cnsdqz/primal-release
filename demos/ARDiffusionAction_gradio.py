#!/usr/bin/env python3
"""
Autoregressive Diffusion Action Motion Generation Gradio Interface

This module provides a Gradio web interface for generating human motion sequences
using autoregressive diffusion models conditioned on action labels.
"""
...
def main():
    ...
    args = tyro.cli(LaunchArgs, description="Launch Gradio interface for action-conditioned motion generation")
...
class LaunchArgs:
    ...
    output_dir: str = "outputs/gradio_ARDiffusionAction"
...
class Constants:
    ...
    ACTION_LIST = ['jump', 'kick', 'punch', 'run', 'walk']
...
class ModelManager:
    ...
    def load_model(self) -> ARDiffusionAction:
        ...
        self.model = ARDiffusionAction.load_from_checkpoint(
            latest_ckpt,
...
class MotionGenerator:
    ...
    def __init__(self, model: ARDiffusionAction, args: LaunchArgs):
        self.model = model
        self.args = args
    ...
    def _set_action_label(self, action: str) -> torch.Tensor:
        """Set action label tensor."""
        action_label = Constants.ACTION_LIST.index(action)
        return torch.tensor(action_label)[None, None].long().to(self.model.device)
    ...
    def _save_results(self, betas: torch.Tensor, xb_gen: torch.Tensor, 
        ...
        # Save SMPL file
        smpl_path = save_data_smplcodec(self.args.output_dir, idx, betas, xb_gen, self.model.fps)
    ...
    def generate(
        ...
        # Set action label
        batch["action_label"] = self._set_action_label(action)
        
        # Generate motion
        betas, xb_gen, kpts, output_logs, _ = self.model.generate_perpetual_navigation(
            batch,
...
class GradioInterface:
    ...
    ARTICLE = """
This interface generates action-conditioned human motions using the PRIMAL action model. The system can create realistic motion sequences based on specific action labels with various control options.
...
3. **Control Parameters**:
   ...
   - **Action**: Select from available actions (jump, kick, punch, run, walk) to generate specific motion behaviors
...
4. **Generation Settings**:
   ...
   - **Guidance Weights**: Control strength for movement velocity, facing direction, and action conditioning (active only when control is enabled)
...
- **Generation Logs**: Technical information about the generation process including the selected action

The model generates action-conditioned motions using classifier-free guidance, producing realistic human movements that follow the specified action semantics while respecting directional and velocity constraints.
"""
    ...
    def create_interface(self) -> gr.Interface:
        ...
        return gr.Interface(
            ...
            title="Perpetual Motion Generation with Semantic Action Control",
    ...
    def _create_inputs(self) -> list:
        ...
        return [
            ...
            gr.Dropdown(label="Action (via semantics)", choices=Constants.ACTION_LIST),
            gr.Slider(
            ...
            gr.Slider(
                ...
                label="Guidance weight of action (Scale for CFG)"
            ),
        ]
